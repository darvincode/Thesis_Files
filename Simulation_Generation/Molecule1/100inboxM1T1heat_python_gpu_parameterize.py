...
dimer_openff = Molecule.from_file('darvin_mol_1_set_ang.mol')
...
dir_name = '100M1T1BBNEW_' + str(now)
...
arr = get_coords_packmol('../molecule_100packed70.xyz')
...
subprocess.run(['gmx', 'grompp', '-f', '../min.mdp', '-c', 'interchange_gromacs.gro', '-p', 'interchange_gromacs.top', '-o', 'interchange_gromacs.tpr', '-maxwarn', '10'], check=True)
...
from datetime import datetime
now = datetime.now().strftime("%d-%m-%Y_%H_%M_%S")
dir_name = 'simulationm1_' + str(now)
os.makedirs(dir_name, exist_ok=True)
